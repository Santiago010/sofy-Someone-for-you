...
export const ModalInfoUser2: FC<ModalInfoProps> = ({
  modalVisible,
  toggleModal,
  user,
  originScreen,
      ...
      <View style={styles.modalOverlay}>
        <View style={styles.modalContent}>
          <ScrollView
            ...
            {user.fromIndividual.individualFiles &&
            ...
            <Text style={styles.modalTitle}>
              {user.fromIndividual.name} {user.fromIndividual.lastname}
            </Text>
            <Text style={styles.modalAge}>{user.fromIndividual.age} years</Text>
            {/* Carrusel Parallax de Imágenes */}
            {user.fromIndividual.individualFiles &&
            user.fromIndividual.individualFiles.length > 0 ? (
              <View style={styles.carouselContainer}>
                <Carousel
                  loop
                  width={Dimensions.get('window').width * 0.7}
                  height={200}
                  data={user.fromIndividual.individualFiles}
                    ...
                    <View style={styles.carouselItem}>
                      ...
                      <View style={styles.imageIndicator}>
                        <Text style={styles.imageIndicatorText}>
                          {index + 1} /{' '}
                          {user.fromIndividual.individualFiles.length}
                        </Text>
                      </View>
                    </View>
                  )}
            ...
            <Text style={styles.modalBiography}>
              {user.fromIndividual.description}
            </Text>
            {/* Mostrar intereses con Chips */}
            {user.fromIndividual.categories &&
              user.fromIndividual.categories.length > 0 && (
                <View style={styles.interestsContainer}>
                  <Text style={styles.interestsTitle}>Interest</Text>
                  <View style={styles.interestsChipsContainer}>
                    {user.fromIndividual.categories.map(category => (
                      <Chip
                        key={category.id}
